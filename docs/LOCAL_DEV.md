...
管理端上传页支持“暂存上传 -> 解析预览 -> 分块确认 -> 确认入库”。暂存文件位于 `data/storage/_staged/<staged_doc_id>/`，DOCX/PDF 内嵌图片会保存为图片资产并在预览页展示；PDF 中 JPEG2000 等浏览器支持不稳定的图片会转为 PNG。预览页展示的段落、表格和图片结构块不会逐个直接入库，后端会先聚合同页或同章节的连续文本，再按语义边界生成分块。未配置 OCR 时，图片资产只提供原图复核与图片编号引用；图片中的文字不会自动进入正文语义检索。
...
- 检查 `.env` 中 `VLLM_ENABLED=true` 是否已启用；正常回答现在必须调用 LLM，不再使用检索片段拼接兜底
