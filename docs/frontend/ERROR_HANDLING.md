...
- 问答页遇到 `CONVERSATION_NOT_FOUND`，或 `AUTH_FORBIDDEN` 且错误信息为“无权访问该会话”
  - 自动刷新当前知识库的会话列表
  - 清理失效会话上下文，避免继续向已删除会话发送请求
  - 保留用户刚输入的问题文本，方便重新发起新会话
...
- 当 `error.code=CHAT_RUN_CANCELED` 或 `done.status=canceled`：
  ...
  - 保留已生成 token 供用户继续查看
  - 允许用户直接再次发送
- 用户主动点击停止后，应先调用 `POST /chat/runs/{run_id}/cancel`；若后续收到 `done.status=canceled`，发送状态回到空闲态。
- 流式连接中断时，如果已经收到 `run_id`，应查询 run 状态并刷新对应会话消息，避免本地“生成已中断”占位与服务端已保存回答重复展示。

## 7.2 流式认证刷新
- SSE 请求由 `fetch` 发起，必须显式复用认证刷新逻辑，不能只依赖 Axios 拦截器。
- 首次流式响应若返回 401，应先尝试 refresh access token，然后用同一业务请求重试一次。
- refresh 失败时按认证错误处理，保留用户输入，避免丢失当前问题。
