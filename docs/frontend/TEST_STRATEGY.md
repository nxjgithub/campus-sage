...
## 布局层回归补充（2026-03）
- 新增门户壳层测试，覆盖匿名态、登录管理员态、端口切换、导航选中与路由元信息触发的特殊布局。
- 布局层测试优先使用数据路由（`createMemoryRouter`）验证 `handle.layout` 等路由元信息行为。

## 上传测试补充（2026-03 第四轮）
- 文档上传测试需覆盖 `txt/docx/html/md` 至少一种成功路径。
- 不支持格式测试改为校验真正未支持的后缀，例如 `exe`。
