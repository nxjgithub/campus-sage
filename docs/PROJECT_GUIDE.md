...
- 管理端创建和评测集录入页采用主表单 + 侧栏摘要/步骤结构；文档上传页已升级为暂存上传、版式化解析预览、分块确认、确认入库的工作台。
- 通用 `loading / empty / error` 状态采用统一工作台壳层，并保留错误码与 `request_id` 的可追踪展示。
- 2026-05 全页面视觉巡检已补齐：后台 compact 页头统一减重，管理端侧栏对齐问答端 `272px` 三段式比例，侧栏长说明两行截断，会话回放页改为面板内部滚动，登录页移动端优先展示账号表单。
...
## 2.1 最新能力快照（已落地）
- 真实 Embedding 已支持 OpenAI 兼容 HTTP 接入；Compose 入库默认强制使用 HTTP Embedding，并在写入后反查本次 chunk 数量，不通过则任务失败并提示重试。
- Docker Compose 中 TEI 已固定清空失效代理变量，降低本地模型下载或加载被宿主代理配置干扰的概率。
- 生成模型已支持 OpenAI 兼容 HTTP 接入，可直接切换到 DeepSeek 等外部服务。
- 正常回答已强制依赖生成模型：`refusal=false` 的答案必须来自 LLM 调用，模型未启用、不可达或返回空内容时走 `RAG_MODEL_FAILED`，不再用检索片段拼接兜底答案。
- 身份认知问题已进入意图分流：对“你是谁/你能做什么”等系统身份问题，服务端在检索前返回固定 CampusSage 说明，不消耗向量检索、重排或生成模型，也不把该说明伪装成知识库引用。
- 提问推荐已进入意图分流：用户问“我可以问哪些问题”时，服务端会基于当前知识库分块样本调用生成模型生成可直接提问的问题；拒答时也会追加若干来自当前知识库范围的推荐问题，帮助用户转向可回答的问题。
- 图片证据当前作为可复核资产返回前端；DeepSeek 普通 chat 链路只读取文本上下文。若图片内容需要参与回答，应在入库阶段先做 OCR 或图注抽取，再把可追溯文本写入分块上下文。
...
- 意图分流已补充就业服务、简历与模拟面试类短问句识别；类似“AI简历如何制作？”“AI模拟面试”这类包含明确业务对象的问题会进入检索链路，不再因缺少学校或系统全称而提前澄清。
...
## 前端近期更新（2026-03 第一轮）
- 登录页已收敛为品牌说明 + 聚焦表单的双栏结构，便于答辩演示时同时解释产品价值与认证入口。
- 管理端整体视觉已统一到工作台壳层、紧凑页头和低噪声内容面板，减少不同页面之间的风格漂移。
- 通用 `loading / empty / error` 状态已统一保留中文提示、错误码和 `request_id`，便于演示与排障。

## 前端近期更新（2026-03 第二轮）
- 管理端文档页重构为入库工作台，合并上传、当前任务、任务历史与文档操作，降低页面割裂感。
- 评测中心重构为统一工作台，左侧维护评测样本，右侧直接运行与查询结果，不再强调内部运行 ID；运行页现优先读取服务端评测集与运行列表。
...
- 前端流式问答已补齐 401 自动刷新 token 后重试，避免 SSE 请求绕开统一认证续期规则。
- 前端问答页已补齐流式取消与断线恢复测试：取消后按 `done.status=canceled` 收敛状态，断线后用 run 状态和服务端历史替换本地占位。
- 会话运营台已补齐助手历史消息证据详情弹窗，回放时可查看请求 ID、耗时与引用证据。
...
- MySQL schema 已补齐中文表注释和字段注释，便于在数据库管理工具中解释数据含义。

## 后端近期更新（2026-05 知识库治理）
- `GET /api/v1/kb` 列表项已返回 `description` 与完整 RAG `config`，治理端列表可直接展示 TopK、阈值、重排开关和上下文预算，避免因列表契约缺字段把已开启重排误显示为关闭。

## 后端近期更新（2026-04 问答流式）
- 流式问答已接入 vLLM OpenAI 兼容 `stream=true`：启用 vLLM 时前端会收到真实 token 增量；未启用 vLLM 时仍保留兜底切片输出用于本地演示。
- 流式回答在持久化前仍会补齐引用编号并执行生成后拒答判定，确保 SSE 体验不绕开 RAG 证据契约。
- 流式取消已增强：服务端等待 vLLM 增量时会周期性检查 run 取消标记，并在取消或断连后关闭上游模型响应。
...
- 管理端监控页已接入运行时诊断，可在队列指标旁查看服务开关、安全风险与 RAG 运行指标。
- 运行时诊断现可区分 JWT “默认占位密钥”与“长度过短密钥”，便于部署前快速发现认证安全风险。
- 当 `APP_ENV=prod` 时，若 JWT 密钥仍为默认值或长度过短，服务会在启动阶段直接阻断，避免带着危险配置上线。
...
- 文档暂存预览新增 `preview_blocks`，前端可按标题、段落、表格、图片顺序还原文档阅读效果，入库分块仍独立可控；预览文本会先按页或章节聚合再生成语义分块，避免视觉行/短段落直接入库；DOCX 图片条目仅 CRC 异常但字节完整时，后端会尝试恢复读取并继续做图片格式校验，PDF 图片也会抽取为可复核资产，JPEG2000 等格式会转为 PNG。
- 图片资产支持 `local` 与 S3/MinIO 对象存储两种后端；问答引用带图片资产时，前端可通过鉴权接口打开原图复核。
- 图文文档入库时，DOCX/PDF 图片会优先绑定到邻近文本分块；检索命中文本证据后，问答页会在 Markdown 回复下方集中展示去重后的关联图片，图片预览与证据详情弹窗互不抢焦点。
...
## 前端近期更新（2026-05 响应式工作台轮）
- 前端设计要求已重新收敛为企业级响应式工作台：浅灰底、白面板、细边框、低圆角、稳定版心，并统一 `1280 / 1024 / 768` 断点策略。
- `/app/ask` 已调整为自适应聊天工作台：桌面端会话侧栏固定、消息区和输入区共享同一版心，中屏以下收起会话列表并保持输入区可见。
- 管理端通用容器、工具条、录入页和表格补齐响应式兜底；宽表格只在内部横向滚动，页面外层继续禁止横向漂移。
- 问答回复正文已强化 Markdown 文档排版，表格和图片证据在气泡内稳定展示；后台表格继续收敛为浅色表头、紧凑行高、标签截断和操作列不换行的数据网格。
- 后台页头已从重卡片收敛为近似无容器的轻量标题带，统计信息改为内联摘要胶囊，让首屏重心回到表格、筛选和录入主任务。
- 最新响应式巡检补齐管理端品牌区悬停行为、文档表格主列截断和问答侧栏控件收缩，长名称只在控件或单元格内部省略，不再撑破工作台边界。
